...
SamplingIntegrator<Float, Spectrum>::render(Scene *scene,
    ...
    if constexpr (!dr::is_jit_v<Float>) {
        ...
        for (int i = 0; i < scene->emitters().size(); i++) {
            Emitter *ptr = scene->emitters()[i];
            ptr->m_block = new ImageBlock(film->crop_size(),
                                          film->crop_offset(), (uint32_t) 4);
            ptr->m_block->set_offset(film->crop_offset());
            ptr->m_block->set_coalesce(block->coalesce() && spp_per_pass >= 4);
...
SamplingIntegrator<Float, Spectrum>::render_sample(const Scene *scene,
    ...
    auto [spec, valid] = sample(pos, scene, sampler, ray, medium,
               aovs , active);
...
SamplingIntegrator<Float, Spectrum>::sample(const Vector2f& pos,const Scene * /* scene */,
                                            Sampler * /* sampler */,
